...
if (isset($_POST['update-user-login'])) //Если существует логин
{
    ...
    if ($login == '') {exit("Вы не ввели логин. <a href=\"user-update.php\">Назад.</a>");} //Если    логин пустой, то останавливаем
    if (strlen($login) < 3 or strlen($login) > 15) { //проверяем    длину
        exit("Логин должен состоять не менее чем из 3 символов и не более чем из 15. <a href=\"user-update.php\">Назад.</a>"); //останавливаем выполнение сценариев
    }
//    проверка на существование пользователя с таким же логином
    $result = mysqli_query($db, "SELECT id FROM users WHERE login='$login'");
    $myrow = mysqli_fetch_array($result);
    if (!empty($myrow['id'])) {
        exit("Извините, введённый вами логин уже зарегистрирован. Введите другой логин. <a href=\"user-update.php\">Назад.</a>"); //останавливаем выполнение сценариев
    }
...
else if (isset($_POST['update-user-password'])) //Если существует    пароль
{
    ...
    if ($password == '') {exit("Вы не ввели пароль. <a href=\"user-update.php\">Назад.</a>");} //если    пароль не введен, то выдаем ошибку
    if (strlen($password) < 3 or strlen($password) > 15) { //проверка на    количество символов
        exit("Пароль должен состоять не менее чем из 3 символов и не более чем из 15. <a href=\"user-update.php\">Назад.</a>"); //останавливаем выполнение сценариев
    }
...
else if (isset($_FILES['update-user-avatar']['name'])) //отправлялась    ли переменная
{
    if (empty($_FILES['update-user-avatar']['name'])) {
...

        {
            ...
            exit("Аватар должен быть в формате <strong>JPG,GIF или PNG</strong>. <a href=\"user-update.php\">Назад.</a>");
    ...
    if ($result4 == 'TRUE') { //если верно, то отправляем на личную страничку
      echo "<html><head><meta http-equiv='Refresh' content='5; URL='user-page.php?id=" . $_SESSION['id'] . "'></head><body>Ваша аватарка изменена! Вы будете перемещены через 5 сек. Если не хотите ждать, то <a href='user-page.php?id=" . $_SESSION['id'] . "'>нажмите    сюда.</a></body></html>";} //отправляем    пользователя назад      
}
?>
