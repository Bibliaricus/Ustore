...
if (empty($myrow['id'])) {
    ...
    if ($ip == $tmp[0]) { //проверяем, есть ли пользователь в таблице "checkAutoRegister"
        ...
        mysql_query("UPDATE checkAutoRegister SET col=$col,date=NOW() WHERE    ip='$ip'");
    } else {
        mysql_query("INSERT INTO checkAutoRegister (ip,date,col) VALUES    ('$ip',NOW(),'1')");
...
// ---------------------------------------------------------------------------------------------------------------
// In down is legacy code, who must be deleted (by I don't do it)
    
// подключаемся к базе
// include ("bd.php");// файл bd.php должен быть в той    же папке, что и все остальные, если это не так, то просто измените путь
//     $result = mysqli_query($db, "SELECT * FROM users WHERE login='$login'"); //извлекаем из базы все данные о пользователе с    введенным логином и паролем
//     $myrow = mysqli_fetch_array($result);
